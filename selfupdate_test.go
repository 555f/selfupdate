...
func TestUpdaterWithEmptyPayloadNoErrorNoUpdate(t *testing.T) {
	...
	mr.handleRequest(
		func(url string) (io.ReadCloser, error) {
			equals(t, "http://updates.yourdomain.com/myapp/"+runtime.GOOS+"-"+runtime.GOARCH+".json", url)
...
func TestUpdaterWithEmptyPayloadNoErrorNoUpdateEscapedPath(t *testing.T) {
	...
	mr.handleRequest(
		func(url string) (io.ReadCloser, error) {
			equals(t, "http://updates.yourdomain.com/myapp%2Bfoo/"+runtime.GOOS+"-"+runtime.GOARCH+".json", url)
			return newTestReaderCloser("{}"), nil
		})
